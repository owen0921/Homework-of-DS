...
Node* build_binary_tree(int inorder[], int preorder[], int instart, int inend) { 		
	...
	int inindex = instart; // search the current root in the inorder 
	while(inorder[inindex] != root->data){
		inindex++;
	}
	root->left = build_binary_tree(inorder, preorder, instart, inindex - 1);
	root->right = build_binary_tree(inorder, preorder, inindex + 1, inend);
...
int main()
{
	...
	printf("\n\ntree2 preorder traversal: 5, 9, 3, 8, 4, 7, 6 \n");
